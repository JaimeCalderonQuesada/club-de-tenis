...
if(isset($_GET["registrar"])){
    ...
    $usuario_id=$data->usuario_id;
        ...
        $sqlEmpleaados = mysqli_query($conexionBD,"INSERT INTO registrar(clase_id,usuario_id,asistido) VALUES($clase_id,$usuario_id,0) ");
        if(mysqli_error($conexionBD)){
...
// Actualizar rgistro
if(isset($_GET["actualizarRegistro"])){

  $data = json_decode(file_get_contents("php://input"));
  $clase_id=$data->clase_id;
  $usuario_id=$data->usuario_id;
  $asistido=$data->asistido;

  if($clase_id != ""){
      $sqlEmpleaados = mysqli_query($conexionBD,"UPDATE `registrar` SET `asistido`='$asistido' WHERE `registrar`.`clase_id`=".$clase_id." AND `registrar`.`usuario_id`= ".$usuario_id);
      echo json_encode(["success"=>1]);
      exit();
  }
